  ...
  it('accent-color: inherit;', async () => {
    ...
    expect(await transformSvelte(html)).toMatchInlineSnapshot(`
      ...
      <form class=\\"pt-[1rem]\\" on:submit|preventDefault={go}>
      	...
      	<button class=\\"font-inherit text-inherit px-[2em] py-[1em] text-[#ff3e00] bg-[rgba(255,62,0,0.1)] rounded-[2em] border-[2px_solid_rgba(255,62,0,0)] outline-none w-[200px] tabular-nums cursor-pointer pt-[1rem] focus:border-[2px_solid_#ff3e00] active:bg-[rgba(255,62,0,0.2)]\\" type=\\"submit\\">
      		GO
      	</button>
      </form>
      "
